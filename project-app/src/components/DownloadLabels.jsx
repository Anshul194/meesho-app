...
export default function DownloadLabels() {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const response = await fetch(
        `${API_ENDPOINT}/api/v1/clients/labels/all?clientId=${selectedClient}&page=${page}&limit=${rowsPerPage}&from=${startDate}&to=${endDate}&marketPlace=${marketPlace || ""}`,
        {
          headers: {
  ...
  const downloadManifest = async (id) => {
    try {
      ...
      if (manifests && manifests.length > 0) {
        ...
        await DownloadLabels1(selectedOrderIds);
      } else {
  ...
  const DownloadLabels1 = async (selectedOrderIds) => {
    try {
      ...
      const token = localStorage.getItem("token");
      const response = await fetch(
        `${API_ENDPOINT}/api/v1/clients/downloadLabels`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (response.ok) {
        ...
        fetchOrders();
        setSnackType("success");
        setSnack("Labels downloaded successfully");
        handleClick();
